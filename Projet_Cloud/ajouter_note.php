...
<nav class="navbar navbar-expand-lg container-fluid navbar-light" style="position: relative; background-color: #E3DCD6;">
    ...
    <div class="titre_notre" style="display: block; height: 300px; background-color: #8C5627;">
        <div class="container">
            <h2 class="text-light" style="padding-block-start:50px ;">Formulaire d'ajout de note</h2>
        </div>
...
$resultetudiant = $conn->query($sql1);
  ...
  // Parcourir les résultats et afficher les données de chaque utilisateur
...
$result = $conn->query($sql2);
  ...
  if ($resultetudiant->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              echo (" <option selected  value=".$row["id"].">".$row["nom_module"] ."</option>");
        ...
        <button type="submit" class="btn btn-primary" style="background-color: #8C5627;border: none;">Ajouter le module</button>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $note = $_POST['note'];
      $nomModule = $_POST['module_id'];
      $nomEtudiant = $_POST['etudiant_id'];

      $sql="INSERT INTO note (note, id_etudiant,module_id) VALUES ($note, $nomEtudiant,$nomModule)";
      if ($conn->query($sql) === TRUE) {
        header("Location: gestion_note.php");
        exit;
    } else {
        echo "Erreur lors de l'insertion de la note: " . $conn->error;
    }
